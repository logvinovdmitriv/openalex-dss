...
- Производные таблицы расчета в рабочем pipeline материализуются через DuckDB и
  потоковое чтение: `author_work.csv/parquet`, `indices.csv/parquet` и
  `ratings.csv/parquet` пишутся на диск без возврата больших таблиц во frontend
  или в память API-процесса как Python-списки.
