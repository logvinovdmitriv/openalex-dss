...
def cohort_author_metrics_json(
    ...
    try:
        return cohorts.cohort_author_metrics(cohort_id, run_id=run_id, dump_id=dump_id, fraction_mode=fraction_mode, filters=filters, limit=limit, offset=offset)
...
def cohort_author_metrics_csv(
    ...
    try:
        data = cohorts.cohort_author_metrics_csv(cohort_id, run_id=run_id, dump_id=dump_id, fraction_mode=fraction_mode, filters=filters, limit=limit, offset=offset)
    except cohorts.CohortNotFound as exc:
        raise HTTPException(status_code=404, detail="Cohort not found") from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
