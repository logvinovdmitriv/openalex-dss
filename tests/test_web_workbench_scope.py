...
class WebWorkbenchScopeTests(unittest.TestCase):
    ...
    def test_local_data_no_scope_warnings_are_deduplicated(self) -> None:
        if not shutil.which("node"):
            self.skipTest("node is not available")
        esbuild_check = subprocess.run(
            ["node", "-e", "import('esbuild').catch(() => process.exit(1))"],
        ...
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            entry = tmp_path / "local_data_warning_entry.ts"
            out = tmp_path / "local_data_warning_entry.mjs"
            entry.write_text(
                textwrap.dedent(
                    f"""
                    import {{ localDataNoScopeWarnings }} from {json.dumps(str(ROOT / "apps/web/src/workbench.ts"))};

                    const warnings = localDataNoScopeWarnings(
                      {{
                        scope_status: "implicit_latest_preview",
                        warnings: ["latest preview"],
                        scope_warnings: ["latest preview"],
                      }},
                      {{
                        scope_status: "explicit_scope",
                        warnings: ["explicit warning"],
                      }},
                      {{
                        scope_status: "implicit_latest_preview",
                        scope_warnings: ["table preview"],
                      }},
                    );

                    const expected = ["latest preview", "table preview"];
                    if (JSON.stringify(warnings) !== JSON.stringify(expected)) {{
                      throw new Error(`expected ${{JSON.stringify(expected)}}, got ${{JSON.stringify(warnings)}}`);
                    }}
                    """
                ),
                encoding="utf-8",
