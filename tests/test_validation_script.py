...
class ValidationScriptTests(unittest.TestCase):
    def test_validate_scientometric_mvp_script_runs_and_checks_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(ROOT / "scripts/validate_scientometric_mvp.py"),
                    "--data-dir",
                    str(data_dir),
                    "--reset",
                ],
            ...
            manifest = json.loads((data_dir / "validation/mvp_validation_manifest.json").read_text(encoding="utf-8"))

            self.assertEqual(manifest["status"], "ok")
            self.assertEqual(manifest["run_id"], "validation_scientometric_mvp")
